...
function checkElementQualityDOM(el) {
  ...
  const rect = el.getBoundingClientRect();
  const lineMax = (typeof window !== 'undefined' && window.__UI_CHECKER_CONFIG__?.lineLengthMax) || 80;
...
function checkTypography() {
  ...
  for (const el of document.querySelectorAll('p, h1, h2, h3, h4, h5, h6, li, td, th, dd, blockquote, figcaption, a, button, label, span')) {
    // Skip uichecker's own elements
    if (el.closest && el.closest('.uichecker-overlay, .uichecker-label, .uichecker-banner, .uichecker-tooltip')) continue;
...
if (IS_BROWSER) {
  ...
  styleEl.textContent = `
    ...
    .uichecker-overlay.uichecker-visible {
      animation-play-state: running;
    }
    .uichecker-overlay.uichecker-hover {
      outline-color: ${BRAND_COLOR_HOVER};
      z-index: 100001 !important;
    }
    .uichecker-overlay.uichecker-hover .uichecker-label {
      background: ${BRAND_COLOR_HOVER};
    }
    .uichecker-overlay.uichecker-spotlight {
      z-index: 100002 !important;
    }
    .uichecker-overlay.uichecker-spotlight-dimmed {
      opacity: 0.15 !important;
      animation: none !important;
      filter: blur(3px);
    }
    .uichecker-spotlight-backdrop {
      position: fixed;
    ...
    .uichecker-spotlight-backdrop.uichecker-visible {
      opacity: 1;
    }
    .uichecker-hidden .uichecker-overlay${EXTENSION_MODE ? '' : ':not(.uichecker-banner)'} {
  ...
  function getSpotlightBackdrop() {
    if (!spotlightBackdrop) {
      ...
      spotlightBackdrop.className = 'uichecker-spotlight-backdrop';
  ...
  function showSpotlight(target) {
    ...
    if (window.__UI_CHECKER_CONFIG__?.spotlightBlur === false) {
    ...
    bd.classList.add('uichecker-visible');
  }

  function hideSpotlight() {
    spotlightTarget = null;
    if (spotlightTimer) { clearTimeout(spotlightTimer); spotlightTimer = null; }
    if (spotlightBackdrop) spotlightBackdrop.classList.remove('uichecker-visible');
  }
  ...
  function repositionOverlays() {
    for (const o of overlays) {
      if (!o._targetEl || o.classList.contains('uichecker-banner')) continue;
  ...
  const visibilityObserver = new IntersectionObserver((entries) => {
    for (const entry of entries) {
      const overlay = entry.target._uicheckerOverlay;
      if (!overlay) continue;
      if (entry.isIntersecting) {
        ...
        if (!overlay._revealed) {
          ...
          requestAnimationFrame(() => {
            overlay.classList.add('uichecker-visible');
  ...
  document.addEventListener('transitionend', (e) => {
    ...
    for (const o of overlays) {
      if (!o._targetEl || o.classList.contains('uichecker-banner') || o.style.display === 'none') continue;
  ...
  const highlight = function(el, findings) {
    ...
    outline.className = 'uichecker-overlay';
    ...
    label.className = 'uichecker-label';
    ...
    el.addEventListener('mouseenter', () => {
      isHovered = true;
      outline.classList.add('uichecker-hover');
    ...
    el.addEventListener('mouseleave', () => {
      isHovered = false;
      outline.classList.remove('uichecker-hover');
  ...
  const showPageBanner = function(findings) {
    ...
    banner.className = 'uichecker-overlay uichecker-banner';
    ...
    if (!EXTENSION_MODE) {
      ...
      toggle.addEventListener('click', () => {
        ...
        document.body.classList.toggle('uichecker-hidden', !overlaysVisible);
  ...
  function buildSelectorSegment(el) {
    ...
    if (el.classList && el.classList.length > 0) {
      const classes = [...el.classList]
        .filter(c => !c.startsWith('uichecker-') && !isLikelyHashedClass(c))
        .slice(0, 2);
      if (classes.length > 0) {
  ...
  const scan = function() {
    ...
    overlayIndex = 0;
    const allFindings = [];
    const _disabled = EXTENSION_MODE ? (window.__UI_CHECKER_CONFIG__?.disabledRules || []) : [];
    const _ruleOk = (id) => !_disabled.length || !_disabled.includes(id);

    for (const el of document.querySelectorAll('*')) {
      // Skip uichecker's own elements and any descendants (overlays, labels, banner, nav buttons)
      if (el.closest('.uichecker-overlay, .uichecker-label, .uichecker-banner, .uichecker-tooltip')) continue;
    ...
    for (const f of layoutFindings) {
      ...
      const existing = el._uicheckerOverlay;
      if (existing) {
        const nameRow = existing.querySelector('.uichecker-label-name');
        const detailRow = existing.querySelector('.uichecker-label-detail');
  ...
  if (EXTENSION_MODE) {
    ...
    window.addEventListener('message', (e) => {
      ...
      if (e.data.action === 'remove') {
        ...
        document.body.classList.remove('uichecker-hidden');
      }
      if (e.data.action === 'highlight') {
        ...
        try {
          ...
          if (target) {
            ...
            for (const o of overlays) {
              if (o.classList.contains('uichecker-banner')) continue;
              const isMatch = o._targetEl === target;
              o.classList.toggle('uichecker-spotlight', isMatch);
              o.classList.toggle('uichecker-spotlight-dimmed', !isMatch);
              if (isMatch) {
                // Force the matching overlay visible immediately, don't wait for IntersectionObserver
                o.style.display = '';
                o.style.animation = 'none';
                o.classList.add('uichecker-visible');
                o._revealed = true;
                positionOverlay(o);
              }
            }
      ...
      if (e.data.action === 'unhighlight') {
        hideSpotlight();
        for (const o of overlays) {
          o.classList.remove('uichecker-spotlight');
