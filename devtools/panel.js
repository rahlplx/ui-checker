/**
 * UI Checker v3 — DevTools Panel Controller
 *
 * Displays findings, provides controls for scanning, overlay toggling,
 * cloning pages, and copying components. Uses branded error borders.
 */
...
function handlePortMessage(msg) {
  ...
  if (msg.action === 'clone-result') {
    // Could show a toast in the panel
  }
}
...
btnClonePage.addEventListener('click', () => {
  postToPort({ action: 'clone-page' });
});

btnCopyComponent.addEventListener('click', () => {
  postToPort({ action: 'start-component-picker' });
});
...
const FIX_SKILLS = {
  ...
  'icon-tile-stack':         'distill, arrange',
...
async function formatFindingsForCopy(findings) {
  ...
  if (skills.length) {
    lines.push(`Suggested skills to fix: ${skills.join(', ')}`);
    lines.push('');
  }
...
async function formatSingleFindingForCopy(item, finding) {
  ...
  lines.push('');
  lines.push(`Suggested skill(s) to fix: ${fixSkillFor(finding.type)}`);
  return lines.join('\n');
}
